...
\begin{document}
	...
	\begin{frame}{Формулювання теми та актуальність}
		\begin{itemize}
			...
			\item \textbf{Новизна підходу:} використання нестандартного моделювання із застосуванням нерівномірних сіток дискретизації, що складаються з нулів Чебишева.
	...
	\begin{frame}{Класифікація візуальних артефактів масштабування}
		\begin{itemize}
			...
			\item \textbf{Субпіксельний зсув (Sub-pixel shift)} – небажана просторова трансляція всього зображення. Візуально непомітний, але значно погіршує об'єктивні метрики (PSNR, MSE).
			\item \textbf{Аліасинг (Aliasing)} – «східчастий ефект» на діагональних контурах. Найбільш характерний для методу найближчого сусіда.
			\item \textbf{Розмиття (Blurring)} – втрата чіткості та різкості після масштабування. Притаманний усім методам інтерполяції, особливо білінійному.
		\end{itemize}
		\vspace{0.3cm}
	\end{frame}
...
\begin{frame}{Підготовка датасетів для тестування масштабування}
	\begin{center}
		% Перший блок (Downscale)
		\includegraphics[width=0.55\linewidth]{DownscaleDiagram.drawio.png} \\
		\vspace{0.1cm}
		{\large Для зменшення зображення} \\
		
		\vspace{0.4cm} % Вертикальний відступ між двома блоками
		
		% Другий блок (Upscale)
		\includegraphics[width=0.55\linewidth]{UpscaleDiagram.drawio.png} \\
		\vspace{0.1cm}
		{\large Для збільшення зображення}
	\end{center}
\end{frame}
